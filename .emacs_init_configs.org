...
- refiling things ::
  - refiling tasks ::
    ...
    + IDO (“Interactively DO things”) is nice, it makes for example locating targets easy ::
      - sources ::
...
*** Handling checklists
   ...
   + setup ::
     ...
     (add-to-list 'load-path (expand-file-name "~/Documents/repos/opt/org-contrib/lisp"))
...
*** Logging
   + logging setup ::
     #+begin_src emacs-lisp
     ;; my version
     (setq org-log-done (quote ((time)     ;; keep track of when a todo item was done => when toggled to done, a line ‘CLOSED: [timestamp]’ is inserted just after the headline
     ...
     #+end_src
   + from ==org-todo-keywords== (see config above), with logging settings, a log entry is added whenever a task moves to any of the following states ::
...
*** Add (require) a final newline when saving files
   + utility :: mainly for editing yasnippets (template system for Emacs, allows to type an abbreviation and automatically expand it into function templates)
