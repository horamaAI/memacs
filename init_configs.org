...
- optimize emacs startup time ::
  + run emacs client to emacs server :: with EmacsClient and EmacsAsDaemon
    - notes ::
      + EmacsClient and EmacsAsDaemon allow daemons to run in the background and use clients to interface faster with the daemons => gain in opening emacs
        - emacsClient is opening a new frame to the running daemon
    - run a client to a background daemon :: ~emacsclient -c~ in terminal, or ~emacsclient --create-frame --alternate-editor=""~
      + while emacs client is running with option ==-c==, it waits for the emacsclient process to finish before giving back the terminal => can use ==-n== to skip waiting for the client to close before giving back the terminal:  ~emacsclient -c -n~ (-n is for "no-wait")
    - create a daemon :: ~emacs --daemon~, or ~emacs --daemon=your_daemon_name~ (runs daemons in background), or ~emacs --fg-daemon=your_daemon_name~ (forces the daemon to run in background)
    - running multiple daemons ::
      + possible to run multiple daemons and link to desired one
      + show folder containing daemons' servers sockets :: ~server-socket-dir~ (*variable* => describe variable: ~C-h v server-socket-dir~)
      + list running daemons :: just list content of value returned by ~C-h v server-socket-dir~, eg: ~ls -la /run/user/1000/emacs~
    - closing an emacsclient windows/frame :: since using frames to daemons, ~C-x C-c~ closes the emacsclient frame, not the daemon
    - kill a daemon ::
      + command :: ~emacsclient -e "(kill-emacs)"~ , or with ==-s== option to kill e specific server: ~emacsclient -e "(kill-emacs)" -s your_daemon_name~ (-s is for 'socket-name')
      + kills the main server by default if no ==-s== option
      + needed for example when made changes to configurations, and wants to apply them. Can do the same though without killing the server, by loading/evaluating the changes from an open frame for example.
    - some options to run with emacsclient/emacsasdaemon ::
      + -c, --create-frame :: create new frame instead of current Emacs frame
      + - -a, --alternate-editor=COMMAND :: if Emacs server is not running, run the specified shell command instead (can also be specified with ==ALTERNATE_EDITOR== environment variable)
        - eg: run usual emacs if no daemon available: ~emacsclient -c -a "emacs"~
      + -nw, -t, --tty :: open new Emacs frame on the current terminal
      + -F, --frame-parameters=ALIST :: set parameters of new frame
      + -d, --display=DISPLAY :: tells server to display the files on the given display
    - configurations :: 
      + configure emacsClient in emacs init file ::
        #+header: :tangle yes
        #+begin_src emacs-lisp
        ;; enable emacs server
        (server-start)
        ;; 
        #+end_src
      + set emacs daemon to run on startup ::
        - in linux based OS, use ==systemd== to run emacs at startup: ~systemctl --user enable --now emacs~ (user configuration, system wise: ~systemctl enable --now emacs~)
      + set required launches and aliases in your terminal, eg in bashrc: ~alias emacs='emacsclient -t'~
      + set proper shortcut command, eg for debian, set shortcut to launch ::
        #+begin_example
        ...
        #+end_example
- uniform file formatting (windows+linux) ::
  + purpose ::
    - avoiding end of lines issues with windows vs linux since setting git config crlf can be harmful ([[https://markentier.tech/posts/2021/10/autocrlf-true-considered-harmful/][source]])
    - setting uniform formatting when editing with emacs by using [[https://editorconfig.org/][editorconfig]]
  + editorconfig for emacs ::
    - source (clone first) :: https://github.com/editorconfig/editorconfig-emacs
    - editorconfig root file :: [[$memacs/.editorconfig]]
    - checkout https://editorconfig.org/ for more configs variables, though might not be all available for editorconfig-emacs 
    - eg entries :: (disabled)
      #+header: :tangle no
      #+begin_src emacs-lisp
	(add-to-list 'load-path "~/Documents/repos/opt/elisp/editorconfig-emacs")
	(require 'editorconfig)
	(editorconfig-mode 1)
      #+end_src
  + define function for dealing with windows' crlf :: automatic line ending conversion to unix
    #+header: :tangle yes
    #+begin_src emacs-lisp
...
- memacs folder ::
  ...
  #+end_src
- emacs files to open on startup ::
  + which ones :: 
    - open main agenda file :: 'tasks_list.org'
    - open emacs manual :: ==(info-emacs-manual) ;; C-h r== (first, install 'emacs-common-non-dfsg' on OS)
  + entries ::
    #+header: :tangle yes
    #+begin_src emacs-lisp
    ;; open file on start
...
- recent files shortcut ::
  ...
  #+end_src
- ==evil-mode==, use vim keymap ::
  + clone evil-mode repo: ~~
  + activate evil-mode:
    #+header: :tangle yes
    #+begin_src emacs-lisp
    (add-to-list 'load-path )
    (add-to-list 'load-path (expand-file-name "~/Documents/repos/opt/evil-mode"))
    # (require 'evil)
    # (evil-mode 1)
    #+end_src
- autocompletion ::
  ...
  #+end_src
- editor's display and rendering :: line wrap, display fill column, etc.
  #+header: :tangle yes
  #+begin_src emacs-lisp
...
*** Org babel
  ...
  + org-babel setup ::
    ...
    (setq org-ditaa-jar-path "~/Documents/repos/opt/ditaa/service/web/lib/ditaa0_10.jar")
  ...
  + use ::
    ...
    4. can toggle display of inline images with :: ==C-c C-x C-v==
       - to display inline image in org-mode use ==ATTR_ORG== attribute :: ~#+ATTR_ORG: :width 100~
       - and set variable ==org-image-actual-width== to nil (default is t), to allow inline images sizing:
         #+header: :tangle yes
         #+begin_src emacs-lisp
         ;; my config, to allow inline images sizing with ~#+ATTR_ORG: :width 100~ for example
...
*** Handling checklists
   ...
   + setup ::
     ...
     (add-to-list 'load-path (expand-file-name "~/Documents/repos/opt/elisp/org-mode/org-contrib/lisp"))
...
*** Getting up-to-date org-mode info documentation
  + get up-to-date documentation from git before the regular (out of date) system versions ::
    #+header: :tangle yes
    #+begin_src emacs-lisp
    (add-to-list 'Info-default-directory-list "~/Documents/repos/opt/elisp/org-mode/doc")
    #+end_src
...
*** Use Smex for M-x ido-completion
  ...
  + setup ::
    #+header: :tangle yes
    #+begin_src emacs-lisp
    (add-to-list 'load-path (expand-file-name "~/Documents/repos/opt/elisp/org-mode/smex/"))
...
*** Org-mime for emails (useful for sending tables, fontified source code, and inline images in email)
  ...
  + setup ::
    ...
    #+begin_src emacs-lisp
    ;; org-mime configs
    (add-to-list 'load-path (expand-file-name "~/Documents/repos/opt/elisp/org-mode/org-mime/"))
    (require 'org-mime)
    #+end_src
  + kbd ::
