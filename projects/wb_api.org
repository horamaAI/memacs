...
** SDMX API queries
- notes ::
  + SDMX API :: preferred standard for statistical data exchange in the UN and in many other statistical organizations (such as ==Worldbank==). The standard defines, amongst other things, a common data model and standardized data access interfaces (APIs).
