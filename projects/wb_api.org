...
- metadata advanced API queries ::
  ...
  + metadata queries definitions ::
    - eg of metadata query response: http://api.worldbank.org/v2/sources/2/country/jpn/metadata
      #+begin_src xml
      ...
      #+end_src
    - Source, Concept :: (as already seen previously)
    ...
    - Metatype :: (eg: ~<wb:metatype id="2-alphacode">JP</wb:metatype>~ of concept 'country' Japan)
      ...
      + Metatypes *belong to Concepts*
      + each concept may have one or more Metatypes
    ...
    - Metadata :: retrieves Metadata for any combination of Source, Concept and Metatype, eg: High Income, East Asia & Pacific, United States of America, 2012, etc.
    - Search :: retrieves Metadata by using keywords in the query string, eg: http://api.worldbank.org/v2/sources/2/search/solid%20fuel, http://api.worldbank.org/v2/sources/2/search/burundi
    - can still download metadata with ==downloadformat==: http://api.worldbank.org/v2/sources/2/country/usa;jpn/series/SP.POP.TOTL/metadata?downloadformat=excel
- some more examples :: 
