...
+ containerization ::
  + images, clusters, pods, etc. :: (recap: ==image -> container -> node -> pod -> cluster==)
    - cluster :: (or 'swarm') set of machines or 'nodes', where node can be any type of node:  physical, virtual machines, cloud instances, etc.
    - containers :: slices of the OS on which the apps are run
    - pod :: logical wrapper for container(s)
+ container runtime ::
