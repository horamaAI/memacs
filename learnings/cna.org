    ...
    - ==–userns== options ::
      ...
      + ==keep-id== ::
        ...
        - side effects: podman adds an entry for the user into the containers ==/etc/passwd== file:
          + with keep-id: ~$podman run –rm --userns=keep-id ubi9 grep $UID /etc/passwd~ // dwalsh:*:3267:3267:Daniel Walsh:/:/bin/sh
  ...
  + notes ::
    ...
    - UIDs inside and outside of the container ::
      ...
      + UIDs used in the containers are also used on the host system: running a container using a given UID will use that uid to run its process on the host
        ...
        - => better to override container's default UID:
          + eg: using ==--user== option, run top on the container as uid 1000, a non-root 'toto' user on the host:
            #+begin_example
            ...
            toto
            #+end_example
        - => for security, better to run rootless containers using host non-root user
      + ==/etc/subuid== and ==/etc/subgid== for UIDs and group IDs configuration for rootless containerization ::
        ...
        - host's UID and subuid ::
          + non-root (current, no ==sudo==) user are usually mapped to 1000, and when promoted with privileges (==sudo==) the UID is 0
            #+begin_example
            $ id -u
            1000
            $sudo id -u
            0
            #+end_example
          + /etc/subuid shows that the host's user has a range of 65536 subuids starting from 100000:
            #+begin_example
            $ cat /etc/subuid
            csmart:100000:65536
            #+end_example
          + one can check with ~podman unshare~ (command inside of a modified user namespace) that the user range are applied to the host's users:
            - ~podman unshare~ ::
              #+begin_example
...
- ~podman image rm [options] image […]~ :: remove image
