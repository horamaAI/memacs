...
- issues :: (while working on dotfiles)
  + testing dotfiles in an isolated (containerised, used podman here) environment:
    - vim in containers:
      + not pre-installed, so need to install it first
      + vimrc:
    - podman specific:
      + running podman rootless/rootful environment:
      ...
      + prepare podman container environment for dotfiles testing:
        - commons to available options:
          + run as rootful from host: (~sudo podman run~), needed since dotfiles configs are installing tools
        - option 1 :: run as default "root" user, not really adapted to dotfiles testing, need further configs to be ready for dotfiles testing
          + consequences (good and/or bad):
            - no need to install sudo, but also need to twitch the dotfiles since their install are mainly sudo-based 
          + steps to workaround configs issues:
            - silence out (comment) "keep-alive" checks for sudo ~if sudo -v; then etc.~
          ...
          + containerfile:
            ...
            #+end_example
        - option 2 :: run as pre-defined user
          + consequences:
            - use containerfile to define a user different than default root:
              ...
              + can overcome issue the issue by installing sudo (from the containerfile), that will allow the default user to switch to root to install tools
          + containerfile:
            #+begin_example
            FROM debian:bookworm
