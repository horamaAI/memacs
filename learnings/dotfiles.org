...
** Issues (encountered while working on dotfiles)
*** issues specific to testing dotfiles in an isolated (containerised) environment (podman):
- vim in containers ::
...
- parsing json/yaml using jq and yq ::
  + fetch content from json multiple fields :: eg, fetch content of fields: ~{ .command, .command_for_test }~
    ...
    2. feedback on experimented commands:
       ...
       - things tried ::
         ...
         + other more example of very complex jq formatting tweaks that still landed strings, or other unwanted results:
           ...
           ```
           #+end_example
- adding bash tests to verify proper installation (using [[https://github.com/bats-core/bats-core?tab=readme-ov-file][bats]]) ::
  + store installed components, and use them later to test if everything went well ::
    - to test that installations, and other stuff and everything went well
    - solution presentation ::
      + storage ::
        - 2 global associative arrays :: one for storing installed packages ('INSTALLED_APPS'), another to store commands to use for testing ('INSTALLED_APPS_TEST_CMDS')
          + both arrays share keys '==command==', the command to test if installation went properly, *but also* the command that was used to install the packages:
            1. values of each entry is the list of installed packages (space separated string), eg: ~["apt" -> "pkg1 pkg2 etc."]~
            2. values of each entry is the command to use for testing, eg: ~["apt" -> "command to use for testing"]~
        - both are simple 1D associative array (elements are space separated strings, of packages for example)
          + instead of a 2D one (associative array of array of packages) ~["apt" -> [pkg1, pkg2, etc.]]~, too complicated, especially in bash
      + pass those stored values between functions and subscripts :: eg.: pass them to function 'execute_install_command'
        - both: exported at superscript level, and either sourced in subscripts, or passed as arguments to functions
          + => careful! when running in independent shells and not using function arguments
        - hurdles:
          + for any subscript that installs anything, it needs to return what was installed to parent (recursive too: collect everything installed by subscripts, to return to root)
        - things tried ::
          + [selected] :: have as functions' *mandatory* arguments those associative array (propagate to subscripts with sourcing for the others)
          + same as above, but optional instead (which is possible to achieve when arguments are grouped using quotes, eg: ~execute_install_command "arg1" "arg2_1 arg2_2" "arg3" "etc"~, and counting)
            - pros: since optional, could be skipped
              + check if meets number of mandatory arguments ~if $# -gt 3, then~;
            - cons:
              + counting => additional ifs
          + return values to caller: (instead of passing argument as nameref)
            - pros:
              + no additional if-then, or arguments for that matter;
              + always returns, up to you to use or not
            - cons ::
              + complex to use with associative arrays, eg when concatenating (though ~asso_array[akey]+="a new value"~ seems to work)
                - not shell universal: when returning with ~echo "${installed_pkgs[@]@K}"~ for example, it expands (with ~@K~ parameter) the associative array as a string, which might not work the same on all shells, for example zsh
      + test script :: where stored test data is used
        - ~configs/basic/tests/test.bats~
    - process example:
      #+begin_example
      + profiles/basic/basic.sh // declares global variable 'INSTALLED_APPS' and 'INSTALLED_APPS_TEST_CMDS'
      +   // call to 'execute_install_command' (configs/basic/bin/functions/utils.sh)
      +   // - if runs into an error, then WARN, or ERROR, otherwise store installed in the associated list
      +   execute_install_command "apt" "sudo apt install ${apt_essentials[*]}" INSTALLED_APPS_TEST_CMDS
      +   execute_install_command "npm" "sudo npm install -g" "${npm_essentials[*]}" INSTALLED_APPS_TEST_CMDS
      +   source configs/basic/packages/setup.sh
      +     // sourced, so exported variables will be propagated to subscripts, and can update those
      +     // otherwise, if setup.sh is run as bashed into a subshell, then to update content from caller script then better to chose functions arguments instead of returning from subshell, and retrieve values from parent shell, seems to complex
      +     process_commands_in_yamls yamlscmds INSTALLED_APPS INSTALLED_APPS_TEST_CMDS // configs/basic/bin/functions/utils.sh
      ...
      + // implement same (or similar) for other profiles: ~profiles/*~
      #+end_example
    - other functions used:
      + process_commands_in_yamls: as in ~process_commands_in_yamls yamlscmds~, used to read from yaml, execute install commands read from it, store them and the commands to use for testing (optional)
    - testing ::
      + required ::
        - in schema, add field "command_for_test" that is run to test that installation of referenced package finished properly
      + structure of test:
      + how ::
        - ==for each pkg in pkgs, if 'command_for_test pkg' returns ok, then print OK/check; otherwise KO/cross==
        - suggestions for 'command_for_test' ::
