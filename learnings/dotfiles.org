    ...
    - basic ::
      ...
      + basic.env ::
        - GNU screen custom configs ::
          + change GNU screen's default control sequence prefix ~C-a~ (which is used a lot for other commands) to ~C-m~, and its alias ~M-1~ (see [[file:linux_commands_and_tools.org::#screen_configs][see screen (terminal multiplexer)/basics/screen control sequence prefix]])
