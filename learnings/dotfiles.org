...
- why, how: (add bash tests using [[https://github.com/bats-core/bats-core?tab=readme-ov-file][bats]])
  + chosen solution:
    - global associative array of ==command->[packages]== is exported at superscript level,
      + and for subscripts prefer sourcing than running in an independent shell
    - ==execute_install_command== takes a 3rd optional argument, which is the associative array of ==command->[packages]==
  + to test that installations, other stuff and everything went well
  + super global variable: for those tests purpose, one needs for example to collect installed tools/apps from various subscripts, and use them where the tests are defined, eg: ~configs/basic/tests/test.bats~
