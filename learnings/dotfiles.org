...
- draft ::
  + to include ::
    ...
    - '*.list' ('*.yaml') files for tool specific installations (filed by command to use), eg: ~apt.yaml~ for packages to install with apt, ~npm.yaml~ for npm, ~python2.yaml~, etc.
...
- dotfiles structure :: source of things to do
  ...
  + configs :: 
    - academic ::
      + 'smthing.yaml', and 'smthing' file for specific installs and configs
