...
- dotfiles structure :: source of things to do
  ...
  + configs :: 
    ...
    - basic ::
      + apt.list (file) ::
        ...
        - to install ::
          + build-essential, git, tree, curl, wget, g++, python, java jdk (basic one: default-jre), valgrind
      + bin (folder) :: 
    ...
    - common ::
      ...
      + emacs (file) ? :: what to install in basic emacs split from common ?
        ...
        - basic emacs configs
        - undo-tree, global-undo-tree-mode
      + emacs (folder) ? ::
  ...
  + profiles ::
    ...
    - install.sh ::
      + so that can setup the selected profile, and also have the same basic folders organisation for all profiles  :: all profiles have the same basic folder organisation, and each content will expand from it:
        - opt ::
        - emacs ::
        - repos ::
        - etc. ::
