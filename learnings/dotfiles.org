  ...
  + configs :: 
    - basic ::
      + emacs ? :: what to split from common ?
        - '*.list' for emacs configs (basics)
    - add default compilers and tools (c++, python, java, docker), just in case, so can do basic tasks such as running a basic test environment
      + opt ::
        - create a '*.list' for repos (for *opt*) to clone
    ...
    - common ::
      ...
      + opt :: don't forget cloning additional elisp stuff, eg: ==opt/elisp/org-contrib==
    - academic ::
  + repos ::
    - use submodules 
  + dev-configs ?? :: (wouldn't be better to organise this by tool? eg: vscode, vs, vim, etc.)
    - contains configs for IDEs, formatting, dev utils, etc.
    - additional tools/repos/plugins/etc. are either in 'dev' folder, or any other dotfile/**/*.list, such as ==./common/ide.list==
  + profiles :: allows easy selection of given profile (suppose one needs to configure a basic easy working docker profile: ==basic+common==, or a profile that contains all "dmahoro" minus "private" stuff and "academic", etc.)
    - all have same basic folders organisation :: content varies with respect to profile
      + opt ::
      + emacs ::
      + repos ::
      + etc. ::
      + other profiles have this basic files structure, plus further customisation
    - dmahoro :: configs: ==basic + common + academic + private==
      + env :: 
      + opt :: add further full profile for dmahoro "dev" for example, with c++/cmake/java/docker/etc.
      + repos :: add complete list of repos
      + repos-private :: folder containing private submodules, or with different remote links (direnv ?)
        - work ::
          + company_a ::
          + company_b ::
          + etc. :: 
      + org :: memacs
    - basic ::
      + basic profile to bootstrap a light ready to go environment, eg: for a docker environment to use for tests
    - common :: (is it really needed ?)
    - can create others when needed
  + private ::
    - :: 
    - dmahoro ::
