  ...
  + testing dotfiles in an isolated (containerised, used podman here) environment:
    ...
    - podman:
      + podman is rootless (by default):
        - by default, not able to use sudo (from within the container), even with root user
        - to be able to install packages for example, need to run rootful (from host)
        - not many user environment variables predefined (eg: $USER)
      + workaround: (need for configurations to be added into a Containerfile ?)
        - run as rootful from host: ~sudo podman run~
        - containerfile: manually set $USER since needed for $TARGET_FOLDER for example, install git (even though can also use curl or wget), clone dotfiles, and run build image:
          #+begin_example
          FROM debian:bookworm
          ENV USER=root
          RUN apt update && apt install -y vim git && mkdir repos && cd repos && git clone https://github.com/horamaAI/dotfiles.git
          #+end_example
        - note: if in the containerfile a user other than root is defined (with no further configs such as configuring UIDs and GIDs), still won't be able to run rootful since only root inherits the privileges of the host's UID
