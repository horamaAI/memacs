...
- services/applications communication ::
  + data streaming :: allows single request to return lot more data than would typically be, eg: REST doesn't allow data
    ...
    - data streaming is bi-direction: server -> client, client -> server; since the client is really any service that
      makes the initial request
  + message broker :: server that contains a persistent, append-only log that stores messages (can be seen as a kind of
    server that contains different log files that get updated as new data comes in)
