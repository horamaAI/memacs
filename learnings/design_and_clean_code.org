...
- use tests templates :: tests usually follow some patterns, and some already are available
  - AAA :: Arrange (Setup and initialize objects and environment) - Act (Exercise functionality to be tested) - Assert (can comment each group to easily locate what's needed)
    #+begin_src java
    @Test
