...
- software architectures ::
  + EDA :: event-driven architecture, uses events to trigger and communicate between decoupled services and is common in modern applications built with microservices. eg of event: change in state, update, item being placed in a shopping cart on an e-commerce website, etc.
  + SOA :: service oriented architecture that focuses on discrete services instead of a monolithic design
    - microservices vs SOA ::
      + the main distinction between the two approaches comes down to scope. To put it simply, service-oriented architecture (SOA) has an enterprise scope, while the microservices architecture has an application scope. And in microservices, applications are decoupled and each service can have independent data storage — unlike SOA.
      + other differences ::
        - on dependency :: SOA is more business units dependent, while for microservices each microservice is independent and they don't usually share a data layer
        - design :: SOA focuses on application service reusability, while microservices focus on decoupling
        - software size :: usually larger than in any other architecture, while microservices consist of small independent serice
...
** Microservices
- REST, RPC, GraphQL, Soap, etc. ::
  + general ::
    - REST works based on a blocking request-response mechanism => not always appropriate for microservices communication, check instead gRPC or message brokers for example
    - gRPC cannot always be used when memory is required to be allocated on the receiving end to handle response, because it always needs one
  + used protocols ::
    - REST: http1
...
- DRY vs WET: avoid duplications <=> avoid WET (Write Every Time), always seek DRY (don't repeat yourself), basically avoid boilerplates and duplications (as much as one can, some are inevitable)
...
- signal vs noise :: signal: clean, useful code; noise: what prevents from understanding the code (CYC, poor names, bad comments, duplications, etc.). noise to signal ratio should be lowest as possible (ideally closest to 0)
...
- things to not return ::
  + avoid to ever have to return null value :: 1. not everyone knows the internal implementation, and might end up with null instance; 2. they know about the return null, and they have to implement an if to check if the variables are not null (at least +1 of CYC, but will increase anytime the method returning null is called). So avoid as much as possible to return null
    - eg: instead of returning null for object of type List<String>, prefer returning Collections.emptyList()
  + special values (magic numbers) :: numbers whose purpose isn't obvious, eg: -1, 0, 1, etc.: they might mean something to you, but not to everyone (particularly client code)
    - prefer to create expressive variables, or throw exceptions
- method parameters ::
  + methods with too many arguments ::
    - have higher CYC, are difficult to read and understand, do too many things (split), use too many primitive types when could be using objects
...
- prefer "fail fast" over "fail safe" ::
  + fail fast: immediately report any failure asap (toward the beginning of the method, perfect for debugging), vs fail safe: try to keep the program running (the system will crash anyway, but in a weird state that is difficult to analyze)
  ...
  #+end_src
- return asap :: in case there are many embedded ifs to determine which result to return, it can be easy to forget about the previous conditions and get mixed up in reading the code
...
- fine tune conditions ::
  + eg: from ~if(!doorClosed==false)~ (1. doorClosed already boolean, no need for equality check; 2. boolean variable starts with 'is'; 3. !isDoorClosed makes less sense than isDoorOpen) => ~if(isDoorOpen)~
...
- Note ::
  + don't catch ::
    - too high level exceptions, might catch exceptions we're not supposed to manage (eg: 'Throwable' catches: out of memory, internal error; 'Exception' catches runtime exception, particularly 'null pointer runtime exception' which are usually programming errors, and when caught it's just basically covering up programming mistakes)
  + 'catch' block ::
...
- Notes :: overall, apply first conventions, best practices, solid, design patterns, etc. and then have a better look at the code for further improvements
- cohesion (better when high) :: degree to which elements inside a class or a module (package, system, etc.) belong together (put together fields and methods that are related and co-dependent). class with proper amount of fields (not too many, not too few) has more cohesion than class with just one. SRP != cohesion, but goes together
  + eg of cohesive but not SRP ::
    ...
    #+end_src
- coupling (better when low) :: be careful of abiding by solid principles, set coupling as loose as possible: adhere to SRP, DIP, ISP, etc. (basically all solid principles); increase cohesion; and try to maintain strong encapsulation (keep things as private as possible: to avoid methods/fields being used when not needing to be exposed thus creating unnecessary coupling. Don't make methods just because, particularly with getters and setters)
  #+begin_src java
  ...
  #+end_src
- principle of proximity (or priximity rule) :: organize code as a way of a book or an article (from top to bottom, don't make reader keep scrolling between messed up and organization mixed to find methods: structure code with respect to the way the call to other methods are done. place related methods close)
  #+begin_src java
...
- writing comments can be just a way to patch poorly written code. If code is well written, comments can be unnecessary, which is good
- avoid ::
  + redundant comments :: that doesn't add any information (~String name; // customer name~), basically comment for code with well defined names
  + misleading comments :: remember to update comment when method is, don't always believe outdated comments, fix them or even remove them
  + useless and misplaced comments :: for example using comments as a history for all the changes done on a method, which is not interesting, there are other ways for that (fix/release notes, version control tools, etc.); or todos
  + TODOs comments (~// TODO~ :: be more detailed and concise on what to do (what needs to be done, why, when, by whom; or if using issue tracking then use issue number and add those information in the ticket), ~// TODO: fix this~ are not acceptable either
  + commented out comments :: actual part of code that is commented to cancel it, usually done for later fix that almost never happens
- best practices ::
  + use JavaDoc (with ~/** [new line] ... */~)
  + use comments to compensate for factors outside of your control (third-parties softwares, unstable environments, etc.), eg (paraphrasing): ~// yes, this is weird, but otherwise it's impossible due to other software that does this thing as not expected~
...
*** DAMP tests
- find right balance between DRY and DAMP (Descriptive and Meaningful Phrases): strive for max readability (even if it means some code duplication) :: tests may test the same function (for example, how it integrates with other methods, using different inputs, etc.), so better to keep it isolated in single test class => minimal duplication and obvious (very DRY tests are poorly tested, complex, and less readable)
- focused tests ::
  + SRP :: so that methods are easy to test and the method easy to debug (not test this *and* that), eg: basicMathTest shouldn't test division and multiplication and modulo; better to have one test for each
  + no if else branching :: split into two different test logic
  + should have a CYC of one, fewer points of failure (one assertion => one point of failure, one reason to fail), and stability :: fewer points of failures lead to tests with higher stability, and thus, less time investigating the test than trying to fix the bug => reliable tests, and better overall maintainability
...
- use tests templates :: tests usually follow some patterns, and some already are available
  - AAA :: Arrange (Setup and initialize objects and environment) - Act (Exercise functionality to be tested) - Assert. Can even comment each group to easily locate what’s the testing step being processed
    #+begin_src java
    @Test
  ...
  - BDD (behavior driven design tests, or 'GWT') :: Given (some condition) - When (action is made) - Then (compare with expected result)
    + AAA vs BDD ::
      ...
      - GWT is useful to communicate in a business environment, where work needs to be mapped to business value (business value is mapped by features through requirements)
      - Given When and Then are keywords in various BDD frameworks such as SpecFlow, and Cucumber
      - AAA is simpler for unit tests, some higher user tests (browser interaction testing), and when testing own code or library (setup and just test)
        #+begin_src java
...
- tips ::
  + process :: define rules -> write code (in line with defined rules) -> use static code checker -> submit for code review (with preferably tests) -> merge
...
- SRP :: every function/class/module, have one and only one reason to change (responsabilities examples: UI, logging, a business logic, orchestration, persistence, end users, networking, IO, error handling, etc.)
