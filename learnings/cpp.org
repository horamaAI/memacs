...
- structure of memory address on the stack :: when quadword is split into words for example, the stack can be seen as a stack of words: suppose a quadword ~fedcba9876543210~, on a word stack is: ~fe;dc;ba;98;76;54;32;10~, with ~fe~ (the most significant byte) the top of the stack, and ~10~ the bottom (the least significant byte)
** vocabulary
:PROPERTIES:
:CUSTOM_ID: vocab
:END:
