...
:CUSTOM_ID: vocab
:END:
- "byte" :: a one-byte integer (suffix b)
- "word" :: a two-byte integer (suffix w)
- "doubleword" :: a four-byte integer (suffix l)
...
- register usage :: sixteen 64-bit registers in x86-64: %rax, %rbx, %rcx, %rdx, %rdi, %rsi, %rbp, %rsp, and %r8-r15
  + %rsp :: used as the stack pointer, a pointer to the topmost element in the stack
  ...
  + %rbp :: in 32-bit x86, the base pointer (formerly %ebp, now %rbp) was used to keep track of the base of the current stack frame, and a called function would save the base pointer of its caller prior to updating the base pointer to its own stack frame.
...
- binary operations :: ~leaq S, D~ (load effective address of source into destination); ~add S, D; sub S, D; imul S, D~ (multiply destination by source); ~xor S, D~ (bitwise XOR destination by source); ~or S, D; and S, D~
