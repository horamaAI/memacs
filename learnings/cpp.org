...
- cmp S2, S1 :: set condition codes according to S1 - S2 (or S2 - S1 with intel syntax)
...
- jump instructions ::
  ...
  + for signed comparisons, "jump if" is not really doing the whole operation, it's just checks if signed bit flag is set/unset
  ...
  + for unsigned bytes use below/above instead of greater/less:
...
- leave :: restores %rsp and %rbp: sets %rsp to %rbp, then pops top of stack into %rbp
- ret :: pop return address from stack and jump there
** Arrays
[2024-04-05 Fri 22:03]
- arrays are stored in memory as contiguous blocks of data, with pointer to first element
- arr[i] = 3, can be expressed in x86-64 as (assuming the address of arr is stored in %rax and the index i is stored in %rcx): ~movq $3, (%rax, %rcx, 8)~
- arrays in assembly example ::
  + c++ code ::
    #+begin_src cpp
    int array() {
        int arr[] = {1, 2, 3};
  ...
  + assembly ::
    #+begin_src asm
    array():
            push    rbp ; by convention, %rbp needs to be preserved, it contains the caller's value for %rbp, and will get back that value with ~pop rbp~ at the end of the function
            mov     rbp, rsp ; now %rbp can build a new context for the current function, %rsp is the beginning of the current function's stack frame
    ...
    array(int*):
            push    rbp
            mov     rbp, rsp
            mov     QWORD PTR [rbp-8], rdi  ; the array of int is considered as a ptr
    ...
    array(int*, int):
            ...
            mov     eax, DWORD PTR [rbp-12]
            cdqe	; convert doubleword to quadword: sign-extend dword in the %eax to %rax
            lea     rdx, [0+rax*4] ; %rdx can be seen here as the index, lea will just convert the integer value as an address, factor 4 since dword
            mov     rax, QWORD PTR [rbp-8]
            add     rax, rdx
            pop     rbp
            ret
    caller():
            push    rbp
            mov     rbp, rsp
            sub     rsp, 32  ; set aside some space for this current context/function, so that next function(s) doesn't override it. A function that doesn't call another function usually has no need for this sub instruction
            call    array()
    ...
    .L10:
            leave ; restores %rsp and %rbp
            ret
    #+end_src
- lea :: for ==load effective address==, useful when the address of a particular value in an array is needed, *not* the value itself. lea stores the calculated memory offset in the specified register, eg:
  + suppose a struct ::
    #+begin_src cpp
    struct Point    
    {
         int xcoord; // suppose a 64 bits architecture: xcoord and ycoord are both 4 bytes
         int ycoord; // since x64 => ycoord is at offset 4 bytes
    };
    #+end_src
  + assuming the array's base is in %ebx, and the variable i in %eax
  + to fetch y's value of i (~int y = points[i].ycoord;~): ~mov edx, [ebx + 8*eax + 4]~ (the right side is the =="effective address"==)
    - factor 8 since ==Point== is 8 bytes in size
    - the value of points[i].ycoord is copied into %edx
  + to fetch *the address* of y at index i (~int *y = &points[i].ycoord;~): ~lea esi, [ebx + 8*eax + 4]~
    - the value of points[i].ycoord is *loaded* into %esi
...
** Optimisation
[2024-04-10 Wed 22:43]
- adding an optimisation preprocessing level does reduce the amount of assembly instructions, eg:
  + source code ::
    #+begin_src cpp
  ...
  + with no optimisation ::
    ...
    doadd():
            ...
            ret
    #+end_src
  + with optimisation (gcc ==-O3== optimisation level) ::
    #+begin_src asm
    add(int, int):
            lea     eax, [rdi+rsi] ; no use of buffer addressing, since with the help of lea, [rdi+rsi] does the addition, and the address of the result is transferred to eax
            ret
    doadd():
            mov     eax, 3
            ret
    #+end_src
- eg 2: inlining a function call
  + source code ::
    #+begin_src cpp
  ...
  + with optimisation (-O3)::
    #+begin_src asm
    timesthree(int):
            lea     eax, [rdi+rdi*2] ; genius right, only need to do a product only once
            ret
    dotimestwo():
            mov     eax, 150 ; no even need to call the function, just inline the final result
            ret
    #+end_src
