...
- show state of all instances of an object in one's namespace :: kubectl get <object> 
  + example, show state of all pods :: kubectl get pods --watch
  ...
  + examples of pods' states :: ==ContainerCreating==: still in process of creating a container (pulling image, etc.), ==Running==: well, for running pod, lol
