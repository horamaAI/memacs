...
** include, insert, edit, search, navigate text, paragraphs, headlines (or sections), and items
...
- tasks can be grouped in org files with #+FILETAGS: entry :: applies tags to *all* tags, but to the file only => can use the tag to filter file specific tags
