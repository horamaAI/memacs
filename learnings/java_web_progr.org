...
*** Spring data JPA
[2024-03-24 Sun 01:33]
**** Notes
- eases work of creating a persistence layer, given that it's required to match SQL database schemas with the object models that are created in the code. It can be tiresome when done manually. With JPA, it's possible to generate and export the database schemas based on the entity models from the Java code.
**** JPA schema generation
- notes ::
  + since JPA 2.1, a standard for database schema generation was introduced, and one is then able to control how to generate and export database schemas through a set of predefined configuration properties
    - script ==action== :: to control which DDL commands to generate, with script ==action== configuration option: ~javax.persistence.schema-generation.scripts.action~. Possible to choose from 4 different options: ==none==: doesn’t generate any DDL commands; ==create==: generates only database create commands; ==drop==: generates only database drop commands; ==drop-and-create==: generates database drop commands followed by create commands
    - script ==target== :: 
      ...
      + for each specified script action, need to define the corresponding target configuration:
        #+begin_src example
        javax.persistence.schema-generation.scripts.create-target
        javax.persistence.schema-generation.scripts.drop-target
        #+end_src
    - schema ==source== :: to generate the schema DDL commands from the entity models, we should include the schema ==source== configurations with the ==metadata== option selected:
      #+begin_example
      javax.persistence.schema-generation.create-source=metadata
      javax.persistence.schema-generation.drop-source=metadata
      #+end_example
**** Schema generation with Spring data JPA
- notes ::
  + example ::
    - consider entity models defined in java with spring JPA enabled
- Spring Data JPA Configuration ::
  + to generate the database schema, one needs to pass the schema generation properties to the persistence provider in use. To do this, set the native JPA properties in the configuration file under the ~spring.jpa.properties~ prefix:
    #+begin_example
    spring.jpa.properties.javax.persistence.schema-generation.scripts.action=create
    spring.jpa.properties.javax.persistence.schema-generation.scripts.create-target=create.sql
    spring.jpa.properties.javax.persistence.schema-generation.scripts.create-source=metadata
    #+end_example
  + Spring Data JPA then passes these properties to the persistence provider when it creates the ~EntityManagerFactory~ bean.
- file ~create.sql~ :: as a result, on the application startup, the above configuration generates the database creation commands based on the entity mapping metadata. Furthermore, the DDL commands are exported into the ~create.sql~ file, which is created in our main project folder, eg:
  #+begin_src sql
...
[2024-03-24 Sun 02:31]
- notes ::
  + if using hibernate, one can use its native API, ==SchemaExport==, to generate the schema DDL commands. Likewise, the Hibernate API uses the application's entity models to generate and export the database schema.
  + with Hibernate’s ==SchemaExport==, it's possible to use the ~drop~, ~createOnly~, and ~create~ methods explicitly:
    #+begin_src java
    ...
    #+end_src
  + when the code above is run, the database creation commands are exported into the ~create.sql~ file in the main project folder.
  + ==SchemaExport== is part of the [[https://www.baeldung.com/hibernate-5-bootstrapping-api][Hibernate Bootstrapping API.]]
**** Schema Generation Options
[2024-03-24 Sun 02:46]
- notes ::
  + although schema generation can save time during development, one should only use it for basic scenarios. For instance, one could use it to quickly spin up development or testing databases.
  + in contrast, *for more complex scenarios* like database migration, one should use more refined tooling, like [[https://www.baeldung.com/liquibase-refactor-schema-of-java-app][Liquibase]] or [[https://www.baeldung.com/database-migrations-with-flyway][Flyway]].
...
** Quarkus
[2024-03-15 Fri 21:04]
*** Introduction
- notes ::
  + quarkus is slowly taking the lead over spring, due to its active evolution, but also to its advances in cloud native applications development
  + quarkus vs spring/spring boot ::
    - spring boot :: connects all Spring projects and helps accelerate developer productivity by offering many production-ready integrations, and thus reduces the amount of configuration and boilerplate. And thanks to its convention over configuration approach, which automatically registers default configurations based on the dependencies available at the classpath in the runtime, Spring Boot considerably reduces the time-to-market for many Java applications.
    - quarkus :: similar approach as Spring Boot, but with an additional promise of delivering smaller artifacts with fast boot time, better resource utilization, and efficiency. It’s optimized for cloud, serverless, and containerized environments. But despite this slightly different focus, Quarkus also integrates well with the most popular Java frameworks.
    - comparisons :: both frameworks integrate well with other projects and frameworks, but their internal implementations and architectures are different, for example:
      + Spring Boot offers web capabilities in two flavors: blocking (synchronous call with ==Servlets==, where threads are blocked as long as responses are received) and non-blocking (asynchronous calls with ==WebFlux==).
      + Quarkus also offers both approaches, but unlike Spring Boot, it allows us to *use both* blocking and non-blocking strategies simultaneously. Moreover, Quarkus has the reactive approach embedded in its architecture.
      + both projects has the ability to create native images (binary and platform-specific executables)
    - others (source: reddit) ::
      + mainly quarkus > spring boot ::
        - low memory footprint
        - Hot reloading (in dev mode) is awesome
        - Fast
        - Shallow stack traces (Compared to Spring boot) and comparatively lesser magic than Spring
        - Plenty of modules/extensions (https://code.quarkus.io)
        - Compile-time wiring (unlike Spring's runtime wiring)
        - Fast startup (Kubectl deploy restart comes out in less than a sec)
        - Qute template engine is really nice (https://quarkus.io/guides/qute, For certain projects, we use template engines to render UI)
        - 'Sponsored by Redhat', makes it easy to convince senior management
* Dependency managers
[2024-03-15 Fri 21:05]
** Gradle
...
**** Notes
- better integration than maven ? (eg: is it easier to build embedded projects with gradle <=> is gradle more appropriate for multi builds projects, ie: where one "super" project is building multiple smaller project ?)
...
- notes ::
  + whereas small projects and monolithic applications may contain a single build file and source tree, it is often more common for a project to be split into smaller, interdependent modules. "interdependent" is vital, as you typically want to link the many modules together through a single build.
  + a multi-project build consists of one root project and one or more subprojects.
  + multi-project builds are sometimes referred as multi-module projects. Gradle refers to modules as subprojects.
  + the multi-project build directory structure should look as follows:
    - .gradle ::
      - some folders
    - gradle ::
      - libs.version.toml ::
    ...
    - sub-project-1 ::
      - build.gradle.kts :: each subproject should have its own ==build.gradle.kts== file.
  + multi-project standards :: gradle community has two standards for multi-project build structures: Multi-Project Builds using ==buildSrc==, and Composite Builds
...
** Maven
[2024-05-28 Tue 15:10]
*** Tips and tricks
- local maven repository :: 
  + default location :: ~/home/<user_name>/.m2~ (linux), ~C:\Users\<user_name>\.m2~ (windows)
  + is the usual target folder of maven files install and builds
  + if not on default location, then use ==maven help plugin== to find it: ~mvn help:evaluate -Dexpression=settings.localRepository -q -DforceStdout~
- clearing caches ::
  + sources ::
    - https://www.baeldung.com/maven-clear-cache
  + direct deletion :: eg: ~rm -rf ~/.m2/repository~
  + with ==maven dependency plugin== :: by using ~purge-local-repository~ goal: ~mvn dependency:purge-local-repository~ (in root of Maven project)
    - when run without any additional flags, it downloads artifacts not present in the cache to resolve the dependency tree ("transitive dependency resolution")
    - next, it deletes the local cache and, finally, re-downloads the artifacts
    - to avoid pre-downloading those missing dependencies, use flag ~actTransitively=false~: ~mvn dependency:purge-local-repository -DactTransitively=false~
    - if only want to clear out our cache (no pre-downloading or re-resolving artifacts): ~mvn dependency:purge-local-repository -DactTransitively=false -DreResolve=false~ (flag ~reResolve=false~ makes the plugin avoid re-downloading dependencies)
* Testing
