...
* args and xargs
** Intro
- use :: passing arguments from standard input to other commands. Whereas some commands allow piping (grep for example), others don't (ls, echo for example: can't ~seq 5 | echo~, need to pipe to xargs first: ~seq 5 | xargs echo~)
