...
- examples ::
  + ~sed s/^\(.\{81\}\).*$/\1/~, to keep first 81 chars (80 + new line). '\1' is referring to first occurrence of pattern
  + ~echo 'a-b-' | sed 's/\(b\?\)-/x\u\1/g'~, outputs: 'axxB'
    - ~/x\u\1~ ::
  ...
  + pretty print folders architecture with sed :: ~ls -R | grep ":$" | sed -e 's/:$//' -e 's/[^-][^\/]*\//──/g' -e 's/─/├/' -e '$s/├/└/'~
...
- as there is a .git folder in the repo, when it has submodules, there is a .gitsubmodules folder added to it
- possible to have nested submodules (in that case to clone /super/ parent, use: ~git submodule update –init –recursive~)
*** git subtrees
