...
- yq and yaml :: yaml files parser and processor
  + yaml ::
    - format ::
  ...
  + yq ::
    ...
    - input example :: apt.yaml
      ...
      #+end_example
    - examples ::
      + accessing properties :: (the brackets [] provide all values from a structure)
        ...
        - ~yq '.apt.pkgs[] | select(. == "screen")' configs/basic/apt.yaml~ :: ~git~
      + searching for value :: (
        - ~~
- jq and json :: 
