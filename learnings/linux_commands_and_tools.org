  ...
  + jq/yq and bash scripting ::
    - bash to yq/jq ::
      + using bash variables ::
        - straightforward way (no args to jq) :: ~ax=xoxo && echo "{'a':'$ax'}" | yq .~
        - with --args option ::
          + format :: ~jq --arg jq_var ${bash_var} [options...] filter [files ...]~
          + eg :: ~bash_fruit_var=Banana && jq --arg jq_fruit_var $bash_fruit_var '.[] | select(.name == "apple") | .name |= $jq_fruit_var' fruits.json~
