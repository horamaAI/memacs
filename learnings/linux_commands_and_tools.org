      ...
      + jq nodes ::
        ...
        - create/add node ::
          ...
          + add new property :: ~yq -y '. | .[] | select(.name=="banana") |= . + {"tastes": "amazing"}'  fruits.json | yq -sy~
            ...
            - option ==-s== (slurp) in the second part so that the result (a list of fruit objects) is translated into an array
