...
- yamale ::
  + tool set for yaml files: linter (much stricter than yamllint), schema validator, parallel yaml files processing (perfect when processing lots of files), package importable in python, etc.
- yaml-validator :: handy when working with node.js
** yaml/json schema validators linters, schema based yaml files validators, but also customisable
* git
