...
- IFS :: variable used by the shell to determine word boundaries while splitting a sequence of character strings
  ...
  + IFS and ==read== command ::
    ...
    - note :: (or rather a remainder)
      + bash single vs double redirection :: (==< vs <<==) single is for overwrite, whereas double is for appending
...
- all variables in general are declared implicitly in bash, to explicitly declare an array, use ==declare==: explicit declaration of a simple array: ==declare -a==, declaration of an associative array: ==declare -A==
      ...
      + using a simple array of size n*2 :: where each line contains 2 elements: the key and its value
        - example ::
          ...
          do
            # echo "process $key - $value"
            content+=($key $value);
          done < <(yq '.[] | to_entries | map([.key, .value] | join("=")) | .[]' tmp.yaml)
