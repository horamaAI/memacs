...
- functions, arrow functions, lambdas, etc. ::
  + arrow function :: compact syntax to define a function, no 'return' keyword ~(param: paramType) => {param.doSomething(); param.doAnotherThing(); }~
    - arrow functions != lambdas, the later are anonymous functions in Javascript, whereas arrow functions is just modern way of defining function in js
- JS arrays and iterables ::
  + 'for...of' vs 'for...in' ::
...
- uppermost files and folders (at root of project, also called 'boilerplate files') :: configuration and setup files
  + .browserslistrc :: list of browsers and their versions that the application will support, file used by the build system to build bundles for the supported browsers
  + .editorconfig :: provides standards settings for the editor, eg: format
  + angular.json :: cli configuration file for local install
  + karma.config.js :: sets up karma, a javascript test runner
  + .angular :: used by the compiler for caching to improve compiler performance
- 'src' :: by convention, all source code in 'src'
  + src subfolders ::
    - 'app' :: contains source file specific to the application per se
      ...
      + app subfolders ::
    - 'assets' :: for any images or other assets that need to be included in the application
    - 'environments' :: sets up the build environment: one for dev, one for prod
      + cli picks the appropriate environment based on used flags
  + files ::
  ...
  + decorator functions format :: ~@Adecorator({...})~
    ...
    - decorator object have properties too: eg:
      + 'selector:' :: to find (select) the html elements needed to style; in angular, represents the root directive for the component
        - convention :: use prefixed value for the selector to reference it as part of the application being implemented (eg: 'pm-root' is the root of the element to be styled, and defined in application "project management" ('pm'))
...
- angular modules ::
  + handy for organization and boundaries from context (provided by the components) :: angular modules help organize applications into cohesive blocks of functionality and provide boundaries within the application
    ...
    - create uncluttered application: instead of one big module, use several with respect to Single Responsibility Principle
    - easy to aggregate and export, and share module
    - provide a template resolution environment
    - allow to selectively aggregate classes from other modules and re-export them in a consolidated convenient module. An angular module can be seen as a hub orchestrating the components and features needed for the components they coordinate. It provides a template resolution environment for each component that belong to the module
    - can be eager loaded when application starts, or lazy loaded asynchronously by the router
  ...
  + root application module 'AppModule' :: mandatory module for all angular application
    - each angular application has at least one module, the root module
    - root application module bootstraps AppComponent, which is the component loaded when the application starts
    - as other modules, declares application's root components
  + a module is also used to bootstrap the startup component (AppComponent) :: insert AppComponent selector in the starting point of the application
...
- angular modules capabilities :: (within @NgModule decorator)
  + declares :: component, pipe, and directives that it manages
    - every created component, pipe, and directive must belong to one and only one angular module (components 'owned' by the component, directives and pipes 'used' by the declared components)
    - all declared component, pipe, and directives are private by default, they are only accessible to other components, directives and pipes declared in the same module
    - ng modules provide the template resolution environment for its component templates (component, pipe, and directives templates are resolved within that template)
  + exports :: angular modules, third party modules, component, directive, pipes, to make them available to use
    - exports array of ng module allows to share an ng module's component, pipe, and directives, with other modules (but also export third party modules, and ng modules)
    - export any other component, directive, and pipe if other components need it
    - re-export modules to re-export their components, directives, and pipes (useful when consolidating features for multiple modules to build a convenience or shared module)
    - can export something not in the imports array. The imports array only needs to import the modules required by the components declared in the module, eg: a SharedModule can export ng FormsModule, even though it doesn't need it per se
  + imports (within @NgModule({}) decorator part) :: done to import functionalities from other angular modules, third party modules, and ng modules
    - adding a module to the imports array makes available any component, directive, and pipes defined in that module's exports array
    - imported functionalities can then be used by the components *declared* in the module
    - only import what the components declared in the module need for their templates
    - importing a module doesn't give access to its imported modules, imports are not inherited, eg: if a imports b, and b imports c, for a to use features of c, b needs to export c, otherwise a cannot
    - use imports array to register services provided by angular or third party modules
      + import modules (such as third party modules) into AppModule to ensure their services are registered one time (when application is initialized)
  + bootstraps the root application component :: defining the component needed to display the first template, eg: ~@NgModule({bootstrap: [AppComponent]});~
    - every application must bootstrap at least one component, the root application component, added to the bootstrap array of the root application module
  + provides :: with provider arrays, angular modules can register services for the whole application with the root application injector
    - however, using provider arrays for the root application injector, is no longer recommended; use decorator @Injectable of the service itself instead of providers arrays, eg: ~@Injectable({providedIn: 'root'}) export class ProductService {}~

**** Creating feature modules (splitting AppModule into separate but interdependent modules)
- Notes ::
  + eg: with SRP principle, split the modules into feature modules, eg: basic feature module such as welcome page, feature specific module such as ProductModule/CustomerManagementModule/InvoicingModule/etc., shared module for shared services or components, etc.)
- example ::
  #+begin_src javascript
  import { NgModule } from '@angular/core';
  ...
  @NgModule({
    declarations: [
      ProductListComponent, // <-- HERE, feature specific components, directives and pipes grouped in the module
      ProductDetailComponent,
      ConvertToSpacesPipe
    ],
    imports: [
      RouterModule.forChild([ // <-- HERE, not 'forRoot' of RouterModule anymore, but instead 'forChild' to establishe the routes, since ProductModule is importing array of a feature module
      ...
      SharedModule // <-- HERE, use of shared module, defined in 'Defining shared modules' section
    ]
  })
...
[2023-12-27 Wed 01:30]
- needed to organize commonly used pieces of the application and export them to be available for import
- example ::
  #+begin_src javascript
  import { NgModule } from '@angular/core';
  ...
  @NgModule({
    declarations: [
      StarComponent // <-- HERE, shared components, directives and pipes of the shared module
    ],
    imports: [
      CommonModule
    ],
    exports: [
      CommonModule,// <-- HERE, CommonModule and FormsModule are needed in different parts of the application, thus its inclusion into shared module
      FormsModule,
      StarComponent // <-- HERE, forwarind use of local component to be used
    ]
  })
  export class SharedModule { }
  #+end_src

**** AppModule (Application Module: arrange and gather application modules)
[2023-12-27 Wed 17:13]
- example ::
  #+begin_src javascript
  import { NgModule } from '@angular/core';
  import { BrowserModule } from '@angular/platform-browser';
  ...
  @NgModule({
    declarations: [
      AppComponent, // <-- HERE, basic components declared in AppModule group
      WelcomeComponent
    ],
    imports: [
      BrowserModule,
      ...
      ProductModule // <-- HERE, imported feature module is given as example in previous section 'Creating feature modules'
    ],
    bootstrap: [AppComponent]
  })
  export class AppModule { }
...
[2023-12-20 Wed 10:47]
- notes ::
  + the root angular module bootstraps AppComponent to provide the directive used in the index.html (what index.html needs, by inserting AppComponent selector/directive in the starting point of the application for example)
  + the root application module bootstraps AppComponent, thus making AppComponent the component loaded when the application starts
    ...
    - these transpiled javascript files are then downloaded and processed by the browser (client side) ::
      ...
      + eg ::
        ...
        - transpiled index.html ::
          ...
          #+end_src
- bootstrap array (in angular module) ::
  + every application must bootstrap at least one component, the root application component :: add it to the bootstrap array of the root application module
...
- angular provides 'RouterModule' ::
  ...
  + to know how routes are available to the application, pass route to ~RouterModule.forRoot([])~ or ~RouterModule.forChild([])~, and pass the array of routes as arguments
  + 'RouterModule.forRoot([])' :: establishes routes for the root of the application
    - what it does ::
      + registers the router service provider,
      + declares the router directives (eg: RouterLink, RouterOutlet),
      + exposes configured routes
    - usually a service needs *not* to be registered more than once, but with forRoot the route service is. So, when needed, better using forChild instead
  + ~RouterModule.forChild([])~ :: establishes routes for a feature module
    - what it does ::
      + declares the router directives (eg: RouterLink, RouterOutlet),
      + exposes configured routes
    - with forChild([]), service is not re-registered, just once
...
** Build, test, and deploy with Angular cli
[2023-12-19 Tue 14:40]
- Notes :: 
  + Angular cli :: used to generate code, execute the application, deploy to production, etc.
  + module overview ::
    - generating new angular apps (ng new) ::
      + when creating an angular project, angular cli installs a local angular cli with the created version of angular, in order to avoid future conflicts between the global and the local versions of the cli
    - serving apps (ng serve) ::
      + compiles app :: turn .ts and templates into javascript
      + generates application bundles from the javascript files :: eg: vendor.js that contains imported angular third parties libraries, runtime.js which is the webpack loader (webpack: tool behind the compilation and bundling process), etc.
        - at serving step, bundles files only available in memory, not yet in storage (they are after building)
      + starts local web server :: (listen on port 4200 by default)
      + serves the application from memory ::
      + rebuilds when changes performed :: (stays in watch mode, looking for any changes)
    - generating code (ng generate) ::
      + automatically generates needed files using 'schematic'
      + schematic :: template based code generator that supports complex logic (generating from a kind of blueprint based on the needed component)
      + based on required type of files needed, can generate :: component, template, style sheet, and test files
        - eg: for 'component' generates all 4, for 'service' generates only the component and test files
    - testing apps (ng test) ::
      + ~ng test~ builds the app, launches the browser, and execute the karma test runner
    - building apps (ng build) :: to prepare for deployment
      + bundles should be as small as possible :: => minify or 'uglify' the code by 'tree shaking' (remove any unused pieces)
      + bundles after build have a hash as part of the final name :: eg: 'main.dcad2122...js'
        - done for cache busting :: every code update needs to be redeployed to the server, but with web browser caching, file names need to be different to redownload the latest version (file names won't match ones in the browser's cache)
        - bundles after build to be found in folder :: 'dist'

*** some angular cli cmds (~ng <command> <args> --<options>~)
[2023-12-20 Wed 10:29]
- create new angular app :: ~ng new [anapp] --prefix [anapp_prefix]~ // eg: 'anapp' 'product manager' with prefix 'pm' (for 'product manager')
- serve the application :: ~ng serve [options]~ // eg: ~ng serve -o~, with -o to open link directly after 
- generate :: ~ng g [type_of_file_needed] [component_name] [options]~
  ...
  + new component :: ~ng generate component a-new-component-name~ (no need to suffix with 'Component')
  ...
  + module :: ~ng g m [module_name] -m [target]~ // -m: to add to imports array, basically add new import module in the [target] module
    - eg: ~ng g m shared/shared -m products/product.module~ // create in folder 'shared' new module SharedModule, and import it into ProductModule in folder products
  + interface :: ~ng generate interface an-interface~
  + route guards :: ~ng g g a-guard~
  + directive :: ~ng g d~
  + class :: ~ng g c~
  + enum :: ~ng g c~
  + service :: ~ng g s~
- print help :: ~ng help~ to show all available ng commands, or ~ng <command> --help~ to have more details of a command
...
- what's npm ? :: many things: repository, command line tool, package manager, etc
  + node.js being a server-side scripting framework (as opposed to Angular.js which is a client-side scripting framework), it is mostly appropriate for backend development, unlike angular.js that is used for frontend development and runs on the client (browser). npm is on the other hand a package manager for node.js
...
- webpack :: use dev console of the web browser to display webpack contents (tab 'sources')
  + webpack contains the associated typescript files, and helps for debugging by directly modifying its content
- prefer implementing lazy loading for improved page load performance
- check out angular forms for building and validating user-entry forms
- use angular schematic to modify or create schematics for generating code as needed
